...
from ...shields import DeterministicLogicShield


def _unwrap_first_env(env: Any) -> Any:
    current = env
    # DummyVecEnv/SubprocVecEnv expose envs when available.
    if hasattr(current, "envs") and len(getattr(current, "envs")) > 0:
        current = current.envs[0]
    # Stop unwrapping once we reach GymCityWrapper, which carries shield config
    # and predicate indexing used by DLS initialization.
...
class DLSPPO(SB3PPO):
    """Deterministic logic-shield PPO baseline.

    Training and evaluation both use a hard policy-level safety mask.
    In the thesis 4-action setup, motion actions are masked and renormalized
    onto `stop` whenever the deterministic stop rule fires.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._dls_enabled = False
        self._dls_pred_grounding_index = None
        self._shield_cfg = None
        self._shield = None
        self._init_dls_from_env()

    def _init_dls_from_env(self) -> None:
        if self.env is None:
            return
        try:
            ...
            if not shield_cfg or shield_cfg.get("type") != "dls" or pred_grounding_index is None:
                return
            self._dls_enabled = True
            self._dls_pred_grounding_index = copy.deepcopy(pred_grounding_index)
            self._shield_cfg = copy.deepcopy(shield_cfg)
            safe_action = int(getattr(self.action_space, "n", 4) - 1)
            self._shield = DeterministicLogicShield(
    ...
    def _ensure_shield(self) -> None:
        if self._shield is None and self._dls_enabled and self._dls_pred_grounding_index is not None:
            safe_action = int(getattr(self.action_space, "n", 4) - 1)
            self._shield = DeterministicLogicShield(
                self._dls_pred_grounding_index,
                num_actions=int(getattr(self.action_space, "n", 4)),
    ...
    def _shield_probs_tensor(self, observation: np.ndarray, base_probs: th.Tensor) -> th.Tensor:
        self._ensure_shield()
        if (not self._dls_enabled) or self._shield is None:
            return base_probs
    ...
    def collect_rollouts(self, env, callback, rollout_buffer, n_rollout_steps):
        ...
        assert isinstance(self.action_space, spaces.Discrete), "DLSPPO currently supports discrete action spaces only."
